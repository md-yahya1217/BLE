...
void updateAdvertisingData() {
  ...
  // Convert the string to ASCII decimal representation
  String manufacturerData;
  manufacturerData += (char)0xFF;  // Manufacturer ID byte 1
  manufacturerData += (char)0xFF;  // Manufacturer ID byte 2

  for (size_t i = 0; i < humidityString.length(); ++i) {
    uint8_t asciiValue = humidityString[i];
    manufacturerData += (char)asciiValue;
  }
